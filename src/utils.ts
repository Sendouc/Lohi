...
export async function usersWithAccess(): Promise<any> {
...
export async function patronsFiveDollarsAndOver(): Promise<any> {
  ...
  return (await response.json()).filter((user: any) => user.patronTier >= 2);
}
